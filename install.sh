...
sudo install -m 755 usr/bin/ksession "$BIN_PATH/ksession"

# Copy the man page to /usr/share/man/man1
echo "Installing man page to $MAN_PATH..."
sudo install -m 644 usr/share/man/man1/ksession.1 "$MAN_PATH"

# Copy the bash completion script
echo "Installing bash completion script to $COMPLETION_PATH..."
sudo install -m 644 usr/share/bash-completion/completions/ksession "$COMPLETION_PATH/ksession"
...
cp -r usr/config/sessions/* "$CONFIG_PATH/sessions/"
